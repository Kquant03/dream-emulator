...
class ServerReconciliation {
  ...
  processClientInput(clientId: string, input: Input) {
    ...
    // Send acknowledgment
    this.sendAcknowledgment(clientId, input.sequence)
  }
  ...
  private validateInput(input: Input, client: ClientState): boolean {
    ...
    if (input.type === 'move') {
      const distance = Math.sqrt(
        Math.pow(input.x - client.position.x, 2) +
        Math.pow(input.y - client.position.y, 2)
      )
      const maxSpeed = client.stats.speed * (input.timestamp - client.lastInputTime) / 1000
